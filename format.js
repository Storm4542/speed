...
function showFormatMsg(shareCodeObj) {
  ...
  console.log(`\n提交机器人 @Turing Lab Bot\n`);
    if(shareCodeObj.Bean ){
      const temp = getRandomArrayElements( shareCodeObj.Bean)
      for (let index = 0; index < temp.length; index++) {
        console.log(
          `/submit_activity_codes bean ${temp[index].join("&")}\n`
        );
      }
    }
...
function getRandomArrayElements(arr = [], count = 4) {
  if (arr.length <= 5) {
    ...
    let n = parseInt(arr.length/5)
    let temp = []
    for (let index = 0; index < n; index++) {
        temp.push(arr.slice(index*5,(index+1)*5))
    }
    return temp
  }
}
